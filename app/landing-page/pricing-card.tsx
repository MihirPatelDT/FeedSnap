...
const PricingCard = ({
  ...
  return (
    <div className="border flex flex-col justify-between bg-white/20 rounded-lg h-full p-6 hover:shadow-md text-left relative">
      {isPopular && (
        <div className="absolute top-0 right-0 bg-gray-900 text-white rounded-bl-lg rounded-tr-lg px-2 py-1">
          Popular
        </div>
      )}
      <div>
      ...
      <div>
        <button
          onClick={() => handleClick()}
          className="bg-gray-900 py-2 mt-3 rounded-lg text-white w-full cursor-pointer"
        >
          Select Plan
        </button>
      </div>
